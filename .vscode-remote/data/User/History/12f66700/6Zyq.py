...
def home_page(pokemon_id):
  ...
  if pokemon_id is not None:
    ...
    if selected_pokemon:
      return render_template("home.html",  all_pokemon=pokemon_data2,poke=pokemon_id,mypokes=mypokesdata)
...
def rename_action(pokemon_id):
  ...
  renamed=user.rename_pokemon(pokemon_id,name)
  flash(f'{oldname} renamed to {name}')
...
def release_action(pokemon_id):
  ...
  pokemon=UserPokemon.query.filter_by(id=pokemon_id).first()
  oldname=pokemon.name
  released=user.release_pokemon(pokemon_id)
  flash(f'{oldname} was released. Goodbye {oldname}')
